...
function Posts() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const responce = await appwriteFunctions.commentFunction({ postId, content })
      reset()
    } catch (error) {
      alert(error.message);
    }
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="relative">
            <div className="relative">
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-2 pr-12 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-mground-100 resize-none"
              />
              <div className="absolute right-2 top-2 z-10">
                <button
                  type="submit"
                  className="p-1 text-mground-100 hover:text-fground-100"
                >
                  <Send size={25} />
                </button>
              </div>
